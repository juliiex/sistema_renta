...
<head>
    ...
    <title>Detalles de Asignación de Permiso</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 p-4">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold">Detalles de Asignación de Permiso</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="mb-3">
                        <strong class="block text-sm font-medium text-gray-700">Permiso:</strong>
                        <p>{{ $rolPermiso->permiso->nombre }}</p>
                    </div>
                </div>
            </div>

            <div>
                <h3 class="text-lg font-semibold mb-3 text-gray-700">Detalles del Permiso</h3>
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="mb-3">
                        <strong class="block text-sm font-medium text-gray-700">ID del Permiso:</strong>
                        <p>{{ $rolPermiso->permiso->id }}</p>
                    </div>

                    <div class="mb-3">
                        <strong class="block text-sm font-medium text-gray-700">Descripción:</strong>
                        <p>{{ $rolPermiso->permiso->descripcion ?? 'Sin descripción' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex space-x-4 mt-6">
            @if(!in_array($rolPermiso->rol->nombre ?? '', ['admin', 'propietario', 'inquilino', 'posible_inquilino']))
                <a href="{{ route('rol_permiso.edit', $rolPermiso->id) }}"
                    class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded transition">
                    Editar
                </a>

                <form action="{{ route('rol_permiso.destroy', $rolPermiso->id) }}" method="POST"
                    onsubmit="return confirm('¿Está seguro que desea eliminar esta asignación de permiso?');">
                    @csrf
                    @method('DELETE')
            ...
            <a href="{{ route('rol_permiso.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded transition">
                Volver
            </a>
        </div>
    </div>
