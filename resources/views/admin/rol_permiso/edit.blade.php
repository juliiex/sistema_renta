...
<head>
    ...
    <title>Editar Asignación de Permiso</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 p-4">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold">Editar Asignación de Permiso</h2>
            ...
            <div class="flex">
                <div class="ml-3">
                    <p class="text-sm text-yellow-700">
                        <strong>Advertencia:</strong> Modificar esta asignación de permiso puede afectar las funcionalidades a las que tienen acceso los usuarios con este rol.
                    </p>
                </div>
            </div>
        </div>

        <form action="{{ route('rol_permiso.update', $rolPermiso->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select name="rol_id" id="rol_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    @foreach ($roles as $rol)
                        <option value="{{ $rol->id }}" {{ $rolPermiso->rol_id == $rol->id ? 'selected' : '' }}>
                            {{ $rol->nombre }}
            ...
            <div class="mb-4">
                <label for="permiso_id" class="block text-sm font-medium text-gray-700 mb-1">Permiso</label>
                <select name="permiso_id" id="permiso_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    @foreach ($permisos as $permiso)
                        <option value="{{ $permiso->id }}" {{ $rolPermiso->permiso_id == $permiso->id ? 'selected' : '' }}>
                            {{ $permiso->nombre }}
                        </option>
                    @endforeach
                </select>
