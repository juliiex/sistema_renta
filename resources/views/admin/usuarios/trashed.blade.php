            ...
            <table class="min-w-full bg-white mt-4 shadow rounded-lg overflow-hidden">
                ...
                <tbody>
                    ...
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-4 py-2">{{ $usuario->deleted_at->format('d/m/Y H:i') }}</td>
                        @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('propietario'))
                            <td class="px-4 py-2">
                            ...
                            <div class="flex space-x-2">
                                ...
                                (auth()->user()->hasRole('propietario') && !$usuario->hasRole('admin') && !$usuario->hasRole('propietario')))
                                    @can('eliminar_usuario')
                                        <form action="{{ route('usuarios.force-delete', $usuario->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar permanentemente este usuario? Esta acción no se puede deshacer.');">
                                            @csrf
                                            @method('DELETE')
