...
<body class="bg-gray-100 p-4">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold">Crear Reporte de Problema</h2>
        ...
        <form action="{{ route('reporte_problema.store') }}" method="POST" class="space-y-4">
            ...
            <div class="mb-4">
                ...
                @if(auth()->user()->hasRole('inquilino'))
                    ...
                    <p class="text-xs text-gray-500 mt-1">Como inquilino, solo puedes crear reportes para tu usuario.</p>
                @endif
            </div>
            ...
            <div class="mb-4">
                ...
                @if(auth()->user()->hasRole('inquilino'))
                    <p class="text-xs text-gray-500 mt-1">Solo puedes reportar problemas de apartamentos que tienes contratados.</p>
                @endif
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="descripcion" id="descripcion" rows="4" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Describa el problema detalladamente" required>{{ old('descripcion') }}</textarea>
                <p class="text-xs text-gray-500 mt-1">Máximo 500 caracteres.</p>
            </div>

            <div class="mb-4">
                <label for="estado" class="block text-sm font-medium text-gray-700 mb-1">Estado</label>
                <select name="estado" id="estado" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                    @foreach ($estados as $key => $value)
                        <option value="{{ $key }}" {{ old('estado', 'pendiente') == $key ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
                @if(auth()->user()->hasRole('inquilino'))
                    <p class="text-xs text-gray-500 mt-1">Los reportes nuevos siempre se crean en estado "Pendiente".</p>
                @endif
            </div>
            ...
            <div class="flex justify-end pt-4">
                ...
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition">Guardar Reporte</button>
            </div>
        </form>
    </div>
</body>
