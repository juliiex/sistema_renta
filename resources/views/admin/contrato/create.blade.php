...
<body class="bg-gray-100 p-4">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold">Crear Contrato</h2>
        ...
        <form action="{{ route('contrato.store') }}" method="POST" class="space-y-4" id="form-contrato">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Si no encuentra el usuario deseado, búsquelo primero en la sección de usuarios.</p>
                </div>
                    ...
                    <select name="apartamento_id" id="apartamento_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        ...
                        @foreach ($apartamentos as $apartamento)
                            <option value="{{ $apartamento->id }}" {{ old('apartamento_id') == $apartamento->id ? 'selected' : '' }}>
                                ...
                                ({{ $apartamento->edificio->nombre ?? 'Edificio no asignado' }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="fecha_fin" id="fecha_fin" value="{{ old('fecha_fin', date('Y-m-d', strtotime('+1 year'))) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                </div>
            </div>
            ...
            <div class="space-y-3">
                ...
                <p class="text-sm text-gray-600 mb-2">Dibuje su firma en el recuadro a continuación:</p>
                ...
                <p class="text-xs text-gray-500 mt-1">La firma dibujada se guardará como imagen y será parte oficial del contrato.</p>
            </div>
            ...
            <div class="flex justify-end pt-4">
                <a href="{{ route('contrato.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded mr-2 hover:bg-gray-600 transition">Cancelar</a>
                <button type="button" id="btn-guardar" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition">Guardar Contrato</button>
            </div>
        </form>
    </div>
